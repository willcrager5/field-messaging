...
4. Where does Precept fit in this map? I've treated its capabilities as absorbed into Field's three-layer architecture, but if Precept had unique ecosystem relationships pre-acquisition, we should surface them. Working position (4/15): mirror the BridgeFT Option 1 phased endorsement pattern on the brand side; ecosystem-relationship question still open.
5. **Product-suite to terrain mapping (new 4/15):** With the four offerings locked (Foundation [TBD], Advisor & Firm Experience, Asset Manager OS, BridgeFT WealthTech API), which terrain layers does each offering touch? Foundation consumes layers 1–4 and sits above them — does it have direct competitive overlap at layer 2 (Addepar/Orion) that warrants explicit framing? Asset Manager OS is aimed squarely at layer 9 — does the "distribution intelligence layer" category claim hold up against any emerging competitor, or is it genuinely new category space? The terrain map should eventually show "which offering serves which layer" explicitly.
